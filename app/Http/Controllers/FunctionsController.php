...
class FunctionsController extends Controller
{
    ...
    public function uploadAjax(Request $request)
    {
        ...
        if ($request->file('file'))
        {
            ...
            do {
                $name = sha1(time()) . '.' . $ext;
            } while (file_exists($_SERVER['DOCUMENT_ROOT'] . '/images/users/' . $name));

            do {
                $name2 = sha1(time()).sha1(time()) . '.' . $ext;
            } while (file_exists($_SERVER['DOCUMENT_ROOT'] . '/images/users/' . $name2));
            /*** end check and create new file name*/

            /*** destination file ***/
            $location = $_SERVER['DOCUMENT_ROOT'] . '/images/users/' . $name;
            ...
            if (strtolower(end($test)) == 'jpg' ||
                ...
                self::GenerateThumbnail($_SERVER['DOCUMENT_ROOT'] . '/images/users/' . $name, $_SERVER['DOCUMENT_ROOT'] . '/images/users/' . $name2, 900, 900, $quality = 1);
            } else {

                self::GenerateThumbnailNormal($_SERVER['DOCUMENT_ROOT'] . '/images/users/' . $name, $_SERVER['DOCUMENT_ROOT'] . '/images/users/' . $name2, 900, 900, $quality = 1);
            }
